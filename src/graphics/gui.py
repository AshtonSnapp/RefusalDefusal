...
class Game(Frame):
    ...
    def setupHomeScreen(self):
        ...
        # Place logo image
        Photo(Game.display, "/home/pi/Desktop/RefusalDefusal/src/graphics/Resources/Logo.png",\
            456, 256, WIDTH/2, HEIGHT/2)
...
class Photo(Canvas):
    ...
    def __init__(self, master, _file, width, height, x, y):
        Canvas.__init__(self, master ,width=WIDTH, height=HEIGHT, bg="white")
        self.place(x=0, y=0)

        img = PhotoImage(file=_file)

        self.create_image(x-width/2, y-height/2, anchor=NW, image=img)
        self.image = img
